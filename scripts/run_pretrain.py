...
from dnazen.ngram._ngram_encoder import NgramEncoder
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--cache-dir", type=str, default=None, 
                        help="数据缓存目录，不设置则不使用缓存")
    parser.add_argument("--num-workers", type=int, default=16,
                        help="数据加载器的工作线程数")
    parser.add_argument("--streaming", action="store_true", 
                        help="使用流式数据加载（适用于超大数据集）")
    parser.add_argument("--buffer-size", type=int, default=10000,
                        help="流式数据加载的缓冲区大小")
    parser.add_argument("--ngram-encoder-path", type=str, default=None,
                        help="N-gram编码器文件路径")
    parser.add_argument("--min-ngram-len", type=int, default=2,
                        help="N-gram编码器最小n-gram长度")
    parser.add_argument("--max-ngram-len", type=int, default=5,
                        help="N-gram编码器最大n-gram长度")
    parser.add_argument("--max-ngrams-per-seq", type=int, default=1000,
                        help="每个序列的最大n-gram数量")
    parser.add_argument("--train-ngram", action="store_true",
                        help="是否训练N-gram编码器")
    parser.add_argument("--min-ngram-freq", type=int, default=5,
                        help="N-gram编码器最小n-gram频率")
    parser.add_argument("--ngram-method", type=str, default="tf-idf",
                        help="N-gram编码器方法")
    parser.add_argument("--max-train-sequences", type=int, default=10000,
                        help="N-gram编码器最大训练序列数量")
...
def load_data_from_file(file_path, tokenizer, max_length=512, cache_dir=None):
    ...
    logger.info(f"序列长度统计 (基于前10000条):")
    ...
    def __iter__(self):
        ...
        with open(self.file_path, 'r') as f:
            ...
            if worker_info is not None:
                ...
                logger.info(f"Worker {worker_id}: 处理文件区域 {start/(1024*1024):.2f}MB - {end/(1024*1024):.2f}MB")
                    ...
                    if len(buffer) >= self.buffer_size:
                        ...
                        for seq in buffer:
                            ...
                            if processed_count % 100000 == 0:
                                ...
                                logger.info(f"Worker {worker_id}: 已处理 {processed_count} 条序列，"
                ...
                for line in f:
                    ...
                    if len(buffer) >= self.buffer_size:
                        ...
                        for seq in buffer:
                            ...
                            if processed_count % 100000 == 0:
                                elapsed = time.time() - start_time
                                logger.info(f"已处理 {processed_count} 条序列，"
                                           f"用时 {elapsed:.2f}秒 ({processed_count/elapsed:.2f} 条/秒)")
                            yield self._process_sequence(seq)
                        buffer = []
        ...
        total_time = time.time() - start_time
        logger.info(f"Worker {worker_id}: 数据加载完成，共处理 {processed_count} 条序列，"
