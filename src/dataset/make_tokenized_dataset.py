...
def rm_empty_lines(data_list: list[str]) -> list[str]:
    ...
    logger.info(f"空行移除完成，耗时: {timedelta(seconds=elapsed)}，"
...
def calculate_max_length(data_text, tokenizer, sample_size=1000):
    ...
    logger.info(f"样本序列长度统计: 最小={min(sample_lengths)}, 平均={sum(sample_lengths)/len(sample_lengths):.1f}, 最大={max(sample_lengths)}")
...
def process_batch(batch_texts, tokenizer, temp_dir, batch_idx, max_length):
    ...
    logger.info(f"批次 {batch_idx} 处理完成，耗时: {timedelta(seconds=tokenize_elapsed)}，"
    ...
    for temp_file in tqdm(temp_files, desc="合并批次文件"):
        try:
            ...
            if current_shape != reference_shape:
                logger.warning(f"批次文件 {temp_file} 的形状不匹配: 预期 {reference_shape}，实际 {current_shape}，将跳过此批次")
                skipped_batches += 1
                continue
...
def main(data, tok, out, batch_size, resume, max_length):
    ...
    logger.info(f"数据读取完成，耗时: {timedelta(seconds=read_elapsed)}，"
                f"读取了 {len(data_text)} 行数据")
    logger.info(f"当前内存使用: {get_memory_usage()}")
