...
from utils.datas import get_all_hg38_data, get_useful_gue_data, get_multi_species_data
...
def get_multi_species_all():
    return get_multi_species_data(f"{DATA_DIR}/mspecies/dev.txt") + get_multi_species_data(
...
@click.option(
    ...
    help="Data source: raw or tokenized.",
    default="raw",
)
@click.option(
    "-d",
...
@click.option(
    "--train-method", help="Training method. freq or pmi", type=click.Choice(["pmi", "freq"]), default="pmi"
)
@click.option(
    "--out-freq",
    help="The out directory for frequency. Would not output freq info if not specified.",
    type=str,
    default=None,
)
def main(
    ...
    returns_freq = out_freq is not None
    ...
    if not mem_efficient:
        if source == "raw":
            ...
            with open(data, "r") as f:
                ...
                for line in tqdm.tqdm(lines, "converting data to tokens"):
                    line_ = line.replace("\n", "")
                    if len(line_) > 0:
                        data_tokenized.append([int(n) for n in line_.split(":")])
            del lines  # save mem

        ngram_encoder = NgramEncoder(
            vocab_dict={}, min_ngram_len=min_ngram_len, max_ngram_len=max_ngram_len, max_ngrams=20
        )
        ...
        ngram_freq = ngram_encoder.train_from_file(
            data,
            min_pmi=min_pmi,
            min_token_count=min_tok,
            min_ngram_freq=min_ngram,
