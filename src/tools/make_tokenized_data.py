...
def get_multi_species_all():
    ...
    elapsed = time.time() - start_time
    logger.info(f"多物种数据获取完成，共{len(result)}个序列（开发集:{len(dev_data)}，训练集:{len(train_data)}），"
...
def get_all_data():
    ...
    logger.info(f"数据组成: 多物种数据({len(mspecies_data)}), GUE数据({len(gue_data)}), "
               f"HG38数据({len(hg38_data)})")
...
def get_tokenized_data(
    ...
    for data_type in data_types:
        ...
        logger.info(f"{data_type}类型数据收集完成，获取了{len(data)}个序列，"
                   f"耗时: {timedelta(seconds=type_elapsed)}")
    ...
    logger.info(f"所有数据收集完成，共{len(all_data)}个序列，总长度={total_len}个字符，"
    ...
    # 显示各步骤耗时占比
    logger.info("各步骤耗时占比:")
    logger.info(f"- 数据收集: {data_elapsed/total_elapsed*100:.1f}% ({timedelta(seconds=data_elapsed)})")
    logger.info(f"- 分词器加载: {tok_elapsed/total_elapsed*100:.1f}% ({timedelta(seconds=tok_elapsed)})")
    logger.info(f"- 分词处理: {tokenize_elapsed/total_elapsed*100:.1f}% ({timedelta(seconds=tokenize_elapsed)})")
    logger.info(f"- 移除特殊token: {clean_elapsed/total_elapsed*100:.1f}% ({timedelta(seconds=clean_elapsed)})")
