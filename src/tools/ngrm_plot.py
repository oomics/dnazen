    ...
    try:
        ...
        if '频率' in ngram_df.columns:
            ...
            if len(ngram_df) > 0:
                ...
                # 绘制理论Zipf分布
                plt.loglog(ngram_df['排名'], ngram_df['理论频率'], 'r--', linewidth=2, label='Theoretical Zipf Distribution')
                ...
                if np.sum(mask) > 1:  # 确保有足够的数据点进行拟合
                    ...
                    # 绘制拟合线
                    plt.loglog(ngram_df['排名'], fit_line, 'r-', linewidth=2, 
                              label=f'Fitted Line (slope = {slope:.2f})')
                    
                    # 绘制理论Zipf线
                    plt.loglog(ngram_df['排名'], theory_line, 'g--', linewidth=2, 
                    ...
                    if len(ngram_df) >= 10:
                        ...
                        # 绘制实际频率
                        plt.loglog(top_df['排名'], top_df['频率'], 'bo', alpha=0.7, label='Actual Frequency')
                        
                        # 绘制理论Zipf分布
                        plt.loglog(top_df['排名'], top_df['理论频率'], 'r--', linewidth=2, label='Theoretical Zipf Distribution')
