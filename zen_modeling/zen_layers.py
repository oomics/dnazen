...
from .zen_padding import (index_first_axis,
                          index_put_first_axis, pad_input,
                          unpad_input, unpad_input_only)
# from transformers.models.bert.modeling_bert import BertPreTrainedModel, BertEmbeddings, BertPooler,
...
class BertNgramEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.word_embeddings = nn.Embedding(config.ngram_vocab_size, config.hidden_size, padding_idx=0)
...
class ZenEncoder(nn.Module):
    ...
    This module is modeled after the Hugging Face BERT's :class:`~transformers.model.bert.modeling_bert.ZenEncoder`,
    ...
    def __init__(self, config):
        ...
        self.ngram_layers = nn.ModuleList([copy.deepcopy(layer) for _ in range(config.num_hidden_ngram_layers)])
    ...
    def forward(
        ...
        ngram_attn_mask_expanded = ngram_attention_mask.unsqueeze(1).unsqueeze(
            2)  # shape: [batch, 1, 1, max_ngram_count]
        ngram_attn_mask_expanded = (1.0 - ngram_attn_mask_expanded.to(torch.float32)) * -10000.0
        ngram_attn_mask_expanded = ngram_attn_mask_expanded.to(hidden_states.dtype)
        ...
        if subset_mask is None:
            for i, layer_module in enumerate(self.layer):
                ...
                if i < self.num_hidden_ngram_layers:
                    ngram_hidden_states = self.ngram_layers[i](ngram_hidden_states, ngram_attn_mask_expanded)[0]
                    tmp = torch.bmm(ngram_position_matrix.float(), ngram_hidden_states.float())
                    valid_tokens = []
                    B = tmp.size(0)
                    # 遍历每个样本
                    for i in range(B):
                        count = int(cu_seqlens[i + 1].item() - cu_seqlens[i].item())
                        valid_tokens.append(tmp[i, :count, :])
                    hidden_states = hidden_states + torch.cat(valid_tokens, dim=0).to(hidden_states.dtype)
...
class ZenModel(BertPreTrainedModel):
    """Overall BERT model.
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.encoder = ZenEncoder(config)
    ...
    def forward(
        ...
        ngram_token_type_ids: Optional[torch.Tensor] = None,
        ...
        **kwargs
        ...
        if ngram_token_type_ids is None:
            ngram_token_type_ids = torch.zeros_like(ngram_input_ids)

        embedding_output = self.embeddings(input_ids, token_type_ids,
                                           position_ids)
        ngram_embedding_output = self.ngram_embeddings(ngram_input_ids, ngram_token_type_ids)
...
class ZenForMaskedLM(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.is_decoder:
            warnings.warn(
                'If you want to use `ZenForMaskedLM` make sure `config.is_decoder=False` for '
                'bi-directional self-attention.')

        self.bert = ZenModel(config, add_pooling_layer=False)
