...
def model_input(model_config):
    ...
    return {
        "input_ids": torch.randint(low=0, high=vocab_size//2, size=(batch_size, max_seq_len), device=device),
        "labels": torch.randint(low=0, high=1, size=(batch_size, vocab_size), device=device),
        "attention_mask": torch.rand(size=(batch_size, max_seq_len), device=device),
        "ngram_input_ids": torch.randint(low=0, high=ngram_vocab_size//2, size=(batch_size, max_ngram_len), device=device),
...
@pytest.mark.usefixtures("model_config", "model_input")
def test_zen_forward(model_config, model_input):
    from dnazen.model.bert_models import BertForMaskedLM
    from transformers.models.bert.configuration_bert import BertConfig
    
    model = BertForMaskedLM(model_config)
    model.to(device=device)
    ret = model(**model_input)
    print(ret)